...
export class ImportProducts extends SfdxCommand {
    ...
    public static examples = [
      `$ sfdx ${CMD} -d 00Dxx0000000000 -w 00Dxx0000000000`
    ];

    protected static flagsConfig = {
      dataid: flags.string({char: 'd', required: true, description: messages.getMessage('dataIdDescription')}),
      webstoreid: flags.string({char: 'w', description: messages.getMessage('webstoreIdDescription')}),
      name: flags.string({char: 'n', description: messages.getMessage('nameDescription')})
    };
    ...
    public async run(): Promise<any> {
      ...
      let dataId = this.flags.dataid;
      if (!dataId) {
        throw new Error('you need to supply a file Id: '+ImportProducts.examples);
      }
      ...
      if (!webStoreId) {
          ...
          webStoreId = (await singleRecordQuery({ conn, query: `select Id from WebStore where Name='${this.flags.name}'` })).Id;
      }
