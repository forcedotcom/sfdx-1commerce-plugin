...
export class ImportProducts extends SfdxCommand {
  ...
  public async run(): Promise<any> {
    ...
    if (!dataId) {
      throw new Error(
        "you need to supply a file Id: " + ImportProducts.examples
      );
    }

    if (!this.flags.webstoreid && !this.flags.name) {
      throw new Error(
        "you need to supply a webstore Id or name: " + ImportProducts.examples
      );
    }
    ...
    if (!webStoreId) {
      this.ux.log(
        `Searching for WebStore ID from given name: ${this.flags.name}`
      );
      webStoreId = (
    ...
    this.ux.log(
      `Starting import for WebStore ID: ${webStoreId} to ${conn.baseUrl()}${DEFAULT_PATH}`
    );
